...
export async function getUserPosts(userId) {
  ...
  try {
    ...
    const { data, error } = await supabase
      .from('videos')
      .select('id,title,author,creator,video_url,video,thumbnail,thumbnail_url,created_at')
      .eq('creator', userId) // or use 'author' if that's where user ID is stored
      .order('created_at', { ascending: false });

    if (error) {
      // If table doesn't exist (404) or other error, fall back to demo videos
      if (error.status === 404) {
        console.log('getUserPosts: videos table not found (404) – returning fallback demos for user.');
      }
      // Return a filtered subset of fallback videos (simulate user's content)
      return FALLBACK_VIDEOS.slice(0, 3).map((v, idx) => ({
        ...v,
        id: `user-${userId}-${idx}`,
      }));
    }
    ...
    if (!data || data.length === 0) {
      // No videos for this user; return empty or demo subset
      return FALLBACK_VIDEOS.slice(0, 2).map((v, idx) => ({
        ...v,
        id: `user-${userId}-${idx}`,
      }));
    }
